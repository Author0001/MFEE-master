...
class Data:
    def __init__(self):
        ...
        # 与gaz_alphabet的索引对应，统计出现次数
        self.gaz_count = {1:0}#改动--jie
        self.gaz_split = {}
        self.biword_count = {}
    ...
    def build_gaz_alphabet(self, input_file, count=False):
        ...
        for line in in_lines:
            if len(line) > 3:
                ...
                # seg_list = get_segment_result(word_list)
                # for seg in seg_list:
                #     self.gaz_alphabet.add(seg)
                #     index = self.gaz_alphabet.get_index(seg)
                #     self.gaz_count[index] = self.gaz_count.get(index, 0)  ## initialize gaz count
                w_length = len(word_list)
                entitys = []
                for idx in range(w_length):
                    # 所有词在词典中从后向前匹配，即matched_entity长度越来越短
                    matched_entity = self.gaz.enumerateMatchList(word_list[idx:])
                    # entitys保存了一句话中每个字对应的matched_entity的所有结果，用于下面count
